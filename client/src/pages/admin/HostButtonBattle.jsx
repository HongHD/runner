...
import { Play, Square, Trophy, Users, CheckSquare, Hourglass, MousePointer2 } from 'lucide-react';

export default function HostButtonBattle() {
    ...
    const [difficulty, setDifficulty] = useState(1); // 1, 2, 3 난이도
    ...
    useEffect(() => {
        ...
        currentSocket.on('participant_update', (data) => {
            if (data.type === 'join') {
                setParticipants(prev => {
                    ...
                    return [...prev, { ...data.participant, buzzedAt: null, rank: null, score: 0 }];
                });
        ...
        currentSocket.on('admin_button_battle_update', (data) => {
            setParticipants(prev => prev.map(p => {
                if (p.id === data.participantId) {
                    return { ...p, buzzedAt: data.buzzedAt, score: data.score };
                }
                return p;
            }));
        });
        ...
        currentSocket.on('admin_ranking_data', (data) => {
            ...
            setParticipants(prev => prev.map(p => {
                ...
                return match ? { ...p, rank: match.rank, score: match.score, buzzedAt: match.buzzedAt } : p;
            }));
        });
        ...
        return () => {
            ...
            currentSocket.off('admin_button_battle_update');
    ...
    const handleStartGame = () => {
        ...
        socket.emit('admin_start_game', {
            ...
            gameTitle: '버튼 배틀',
            gameType: 'button_battle',
            settings: { difficulty: parseInt(difficulty) || 1 }
        });
        setGameStatus('playing');
        setParticipants(prev => prev.map(p => ({ ...p, rank: null, buzzedAt: null, score: 0 })));
    };
            ...
            <div style={{ minHeight: '100vh', background: 'var(--rvd-bg, #040914)', color: '#fff', display: 'flex', flexDirection: 'column' }}>
                <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <div>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--rvd-cyan)', marginTop: 4 }}>// BUTTON BATTLE · FINAL RANKING</div>
                    </div>
                    ...
                    {sorted.map((p, i) => {
                        ...
                        return (
                            <div key={p.id || i} style={{
                                ...
                                <div style={{ width: 36, height: 36, borderRadius: '50%', background: hasScore ? 'linear-gradient(135deg,#22c55e,#166534)' : 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 800, color: '#fff', flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                    {gameStatus === 'waiting' && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.05)', padding: '6px 12px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)' }}>
                            <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--rvd-text-dim)' }}>난이도:</span>
                            <div style={{ display: 'flex', gap: 4 }}>
                                {[1, 2, 3].map(lvl => (
                                    <button
                                        ...
                                        style={{
                                            background: difficulty === lvl ? 'var(--rvd-cyan)' : 'rgba(0,0,0,0.3)',
                                            color: difficulty === lvl ? '#000' : '#fff',
                                            border: `1px solid ${difficulty === lvl ? 'var(--rvd-cyan)' : 'rgba(255,255,255,0.2)'}`,
                                            fontWeight: 700, borderRadius: 4, width: 28, height: 28,
            ...
            <main style={{ padding: '24px 32px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 24, marginBottom: 24 }}>
                    ...
                    <div style={{ background: 'rgba(34, 197, 94, 0.05)', border: '1px solid rgba(34, 197, 94, 0.15)', borderRadius: 12, padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                            <div style={{ fontSize: 12, color: 'var(--rvd-text-dim)', marginBottom: 8 }}>점수 제출자</div>
                    ...
                    <div style={{ background: 'rgba(239, 68, 68, 0.05)', border: '1px solid rgba(239, 68, 68, 0.15)', borderRadius: 12, padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                            <div style={{ fontSize: 12, color: 'var(--rvd-text-dim)', marginBottom: 8 }}>미제출 / 게임중</div>
                ...
                <div style={{
                    ...
                    {displayParticipants.map((p, i) => {
                        const isDone = !!p.buzzedAt || (p.score > 0);
                        ...
                        return (
                            <div key={p.socketId || i} style={{
                                ...
                                <div style={{ fontSize: 12, fontWeight: 600, color: isDone ? '#4ade80' : 'var(--rvd-text-dim)', fontFamily: 'Orbitron, monospace' }}>
                                    {formatScore(p.score)}
                                </div>
                            </div>
                        );
