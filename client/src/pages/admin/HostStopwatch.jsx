...
export default function HostStopwatch() {
    ...
    const [targetTime, setTargetTime] = useState('10.00'); // 스탑워치 목표 시간
    ...
    useEffect(() => {
        ...
        currentSocket.on('admin_stopwatch_update', (data) => {
            setParticipants(prev => prev.map(p => {
                if (p.id === data.participantId) {
                    return { ...p, buzzedAt: data.buzzedAt, elapsedMs: data.elapsedMs };
                }
                return p;
            }));
        });
        ...
        return () => {
            ...
            currentSocket.off('admin_stopwatch_update');
    ...
    const handleStartGame = () => {
        ...
        socket.emit('admin_start_game', {
            ...
            gameTitle: '스탑워치',
            gameType: 'stopwatch',
            settings: { targetTime: parseFloat(targetTime) || 10.00 }
        });
            ...
            <div style={{ minHeight: '100vh', background: 'var(--rvd-bg, #040914)', color: '#fff', display: 'flex', flexDirection: 'column' }}>
                <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <div>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--rvd-cyan)', marginTop: 4 }}>// STOPWATCH · FINAL RANKING</div>
                    </div>
                ...
                <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                    {gameStatus === 'waiting' && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.05)', padding: '6px 12px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)' }}>
                            <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--rvd-text-dim)' }}>목표 초:</span>
                            <div style={{ display: 'flex', alignItems: 'center' }}>
                                <input
                                    ...
                                    style={{
                                        width: 70, background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(0,212,255,0.3)',
                                        color: '#fff', borderRadius: 6, padding: '4px 8px', fontSize: 14, fontWeight: 700,
                                        textAlign: 'right', outline: 'none'
                                    }}
                                />
                                <span style={{ marginLeft: 6, fontSize: 12, color: 'var(--rvd-text-dim)' }}>초</span>
                            </div>
                        </div>
                    )}
