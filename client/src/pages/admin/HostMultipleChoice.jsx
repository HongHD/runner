...
export default function HostMultipleChoice() {
    ...
    useEffect(() => {
        ...
        if (sock.connected) {
            sock.emit('admin_join', { pinCode: persistentPin, adminId: admin.id, gameId, gameType: 'multiple_choice' });
        }
        sock.on('connect', () => {
            sock.emit('admin_join', { pinCode: persistentPin, adminId: admin.id, gameId, gameType: 'multiple_choice' });
        });
        ...
        return () => {
            ...
            sock.off('mc_answer_received');
            sock.off('mc_revealed');
            sock.off('mc_final_scores');
            sock.off('connect');
        };
    ...
    const sendQuestion = (idx) => {
        ...
        const tl = (questions[idx]?.time_limit || 20);

        const questionData = {
            pinCode: persistentPin,
            questionIndex: idx,
            gameId: parseInt(gameId),
            options: questions[idx].Options.map((o, index) => ({ text: o.option_text, index: index + 1 }))
        };

        sock().emit('admin_mc_next_question', questionData);
        startTimer(tl);
    };
    ...
    const revealAnswer = () => {
        ...
        const correctAnswer = correctOpt ? correctOpt.option_text : q.Options[0]?.option_text;
        sock().emit('admin_mc_reveal', { pinCode: persistentPin, questionIndex: currentQ, correctAnswer });
        if (timerRef.current) clearInterval(timerRef.current);
        setTimer(0);
    };
    ...
    const endGame = () => {
        ...
        if (timerRef.current) clearInterval(timerRef.current);
        sock().emit('admin_mc_end', { pinCode: persistentPin });
    };
        ...
        return (
            <div style={{ minHeight: '100vh', background: '#040914', color: '#fff', display: 'flex', flexDirection: 'column' }}>
                <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                    <h1 style={{ fontSize: 20, fontWeight: 800, margin: 0 }}>🏆 4지선다 최종 결과</h1>
                    ...
                    <div style={{ flex: 1, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: 24 }}>
                        ...
                        {finalScores.leaderboard.map((p, i) => (
                            <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '10px 14px', borderRadius: 8, background: i < 3 ? 'rgba(251,191,36,0.08)' : 'rgba(255,255,255,0.02)', marginBottom: 6, border: '1px solid rgba(255,255,255,0.05)' }}>
                                ...
                                <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', marginRight: 10 }}>평균 {(p.totalTime / 1000).toFixed(2)}초</span>
                    ...
                    {gameStatus === 'waiting' ? (
                        ...
                        <div style={{ width: '100%', maxWidth: 700, textAlign: 'center' }}>
                            ...
                            {gameStatus === 'revealed' && revealedAnswer && (
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, justifyItems: 'center', justifyContent: 'center', marginBottom: 20 }}>
                                    {currentQuestion.Options.map((opt, idx) => (
                                        <div key={idx} style={{
                                            width: 150, height: 120, borderRadius: 16, padding: 12,
                                            display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center',
                                            fontSize: 20, fontWeight: 800,
                                            background: opt.option_text === revealedAnswer ? 'rgba(59,130,246,0.25)' : 'rgba(255,255,255,0.03)',
                                            border: `3px solid ${opt.option_text === revealedAnswer ? '#3b82f6' : 'rgba(255,255,255,0.08)'}`,
                                            color: opt.option_text === revealedAnswer ? '#3b82f6' : 'rgba(255,255,255,0.3)',
                                            boxShadow: opt.option_text === revealedAnswer ? `0 0 30px rgba(59,130,246,0.3)` : 'none',
                                            scale: opt.option_text === revealedAnswer ? '1.05' : '1',
                                            transition: 'all 0.3s'
                                        }}>
                                            {opt.option_text.length > 20 ? opt.option_text.substr(0, 18) + '...' : opt.option_text} {opt.option_text === revealedAnswer && <span style={{ marginLeft: 8 }}>✓</span>}
                                        </div>
                                    ))}
                                </div>
                            )}
                            ...
                            {/* 답변 현황 */}
                            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', marginTop: 24 }}>
                    ...
                    {participants.map(p => {
                        ...
                        return (
                            <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 8, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', marginBottom: 6 }}>
                                ...
                                {ans && (
                                    <div style={{ width: 28, height: 28, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 900, background: 'rgba(34,197,94,0.2)', color: '#4ade80', border: '1px solid rgba(34,197,94,0.4)' }}>
                                        ✓
                                    </div>
                                )}
                            </div>
